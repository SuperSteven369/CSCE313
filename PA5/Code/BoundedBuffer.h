...
private:
	...
	// for thread-safety
	mutex m;
	condition_variable data_available; // for wait by the pop, signaled by the push 
	condition_variable slot_available; // waited on by the push, signaled by the pop
...
public:
	...
	void push(char* data, int len){
		...
		//3. Then push the vector at the end of the queue, watch our for race condition
		q.push(d);
		l.unlock();
